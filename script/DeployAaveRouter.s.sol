...
import {AaveRouter} from "../src/aave/AaveRouter.sol";
import {AaveDebt} from "../src/aave/AaveDebt.sol";
    ...
    function run() external returns (AaveRouter) {
        ...
        if (chainId == 11155111) {
            aavePoolAddressesProvider = vm.envAddress(
                "AAVE_POOL_ADDRESSES_PROVIDER_TESTNET"
            );
        } else if (chainId == 1) {
            aavePoolAddressesProvider = vm.envAddress(
                "AAVE_POOL_ADDRESSES_PROVIDER_MAINNET"
            );
        } else {
            revert();
        }
