...
echo "🔮 Updating Oracle Prices on Sepolia Testnet"
echo "============================================="
...
if [ ! -f ".env.sepolia" ]; then
    ...
    echo "Please run ./scripts-utils/sepolia/deploy_aave_v3_sepolia.sh first"
    exit 1
fi
...
if [ -z "$ORACLE_MANAGER" ]; then
    echo "❌ Error: ORACLE_MANAGER address not found in .env.sepolia"
    echo "Please redeploy with: ./scripts-utils/sepolia/deploy_aave_v3_sepolia.sh"
    exit 1
fi
...
for i in "${!TOKEN_ORDER[@]}"; do
    ...
    if [ -z "$ORACLE_ADDRESS" ]; then
        echo "❌ Error: Oracle address not found for $ORACLE_VAR in .env.sepolia"
        exit 1
    fi
...
if [ $? -eq 0 ]; then
    ...
    for i in 0 2 3; do  # Check WETH, WBTC, USDC
        ...
        ACTUAL_PRICE=$(cast call $ORACLE_ADDRESS "latestAnswer()" --rpc-url $SEPOLIA_RPC_URL)
