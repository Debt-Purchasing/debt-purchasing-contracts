...
const BRIDGE_RECEIVER_CALLDATA_ABI = [
  "address[]",
  "uint256[]",
  "string[]",
  "bytes[]",
...
async function makeBridgeReceiver({ initialize } = { initialize: true }) {
  ...
  const BaseBridgeReceiverFactory = (await ethers.getContractFactory(
    "BaseBridgeReceiverHarness"
  )) as BaseBridgeReceiverHarness__factory;
  ...
  const localTimelock = await makeTimelock({
    admin: baseBridgeReceiver.address,
  });
...
export function encodeBridgeReceiverCalldata({
  targets,
  values,
  signatures,
  calldatas,
}) {
  return utils.defaultAbiCoder.encode(BRIDGE_RECEIVER_CALLDATA_ABI, [
    targets,
    values,
    signatures,
    calldatas,
...
describe("BaseBridgeReceiver", function () {
  it("is initialized with empty storage values", async () => {
    ...
    expect(await baseBridgeReceiver.govTimelock()).to.eq(
      ethers.constants.AddressZero
    );
    expect(await baseBridgeReceiver.localTimelock()).to.eq(
      ethers.constants.AddressZero
    );
  ...
  it("initializing sets values", async () => {
    const { baseBridgeReceiver, govTimelock, localTimelock } =
  ...
  it("reverts on initialize if local timelock admin is not the bridge receiver", async () => {
    ...
    const badLocalTimelock = await makeTimelock({
      admin: ethers.constants.AddressZero,
    });

    await expect(
      baseBridgeReceiver.initialize(
        govTimelock.address,
        badLocalTimelock.address
      )
  ...
  it("processMessage > reverts if messageSender is not govTimelock", async () => {
    const { baseBridgeReceiver, signers } = await makeBridgeReceiver();
  ...
  it("executeProposal > reverts if proposal is expired", async () => {
    ...
    await baseBridgeReceiver.processMessageExternal(
      govTimelock.address,
      calldata
    );
    ...
    await ethers.provider.send("evm_setNextBlockTimestamp", [
      eta.add(gracePeriod).toNumber() + 1,
    ]);
    ...
    await expect(baseBridgeReceiver.executeProposal(1)).to.be.revertedWith(
      "custom error 'ProposalNotExecutable()'"
    );
  });
  ...
  it("executeProposal > reverts if proposal is already executed", async () => {
    ...
    const calldata = encodeBridgeReceiverCalldata({
      ...
      values: Array(1).fill(0),
      signatures: Array(1).fill("setDelay(uint256)"),
      calldatas: [utils.defaultAbiCoder.encode(["uint256"], [20 * 60])],
    });
  ...
  it("state > reverts for proposal id = 0", async () => {
    ...
    await expect(baseBridgeReceiver.executeProposal(0)).to.be.revertedWith(
      "custom error 'InvalidProposalId()'"
    );
  });
