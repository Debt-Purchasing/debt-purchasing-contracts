...
import {
  encodeBridgeReceiverCalldata,
  makeTimelock,
} from "./base-bridge-receiver-test";

async function makeSweepableBridgeReceiver(
  { initialize } = { initialize: true }
) {
  const [_defaultSigner, govTimelockAdmin, ...signers] =
    await ethers.getSigners();

  const SweepableBridgeReceiverFactory = (await ethers.getContractFactory(
    "SweepableBridgeReceiverHarness"
  )) as SweepableBridgeReceiverHarness__factory;
  ...
  const localTimelock = await makeTimelock({
    admin: sweepableBridgeReceiver.address,
  });
...
async function makeFaucetToken(
  ...
  const token = await FaucetFactory.deploy(
    initialAmount,
    name,
    decimals,
    symbol
...
async function proposeAndExecute(
  ...
  await sweepableBridgeReceiver.processMessageExternal(
    govTimelock.address,
    calldata
  );
...
describe("SweepableBridgeReceiver", async () => {
  it("sweeps standard ERC20 token", async () => {
    const { sweepableBridgeReceiver, localTimelock, govTimelock, signers } =
      await makeSweepableBridgeReceiver();
    const [alice] = signers;
    ...
    await USDC.connect(alice).transfer(
      sweepableBridgeReceiver.address,
      transferAmount
    );

    const oldBridgeReceiverBalance = await USDC.balanceOf(
      sweepableBridgeReceiver.address
    );
    const oldTimelockBalance = await USDC.balanceOf(localTimelock.address);

    await proposeAndExecute(sweepableBridgeReceiver, govTimelock, {
      ...
      calldatas: [
        utils.defaultAbiCoder.encode(
          ["address", "address"],
          [localTimelock.address, USDC.address]
        ),
      ],
    });

    const newBridgeReceiverBalance = await USDC.balanceOf(
      sweepableBridgeReceiver.address
    );
    const newTimelockBalance = await USDC.balanceOf(localTimelock.address);

    expect(newBridgeReceiverBalance.sub(oldBridgeReceiverBalance)).to.be.equal(
      -transferAmount
    );
    expect(newTimelockBalance.sub(oldTimelockBalance)).to.be.equal(
      transferAmount
    );
  });
  ...
  it("sweeps non-standard ERC20 token", async () => {
    ...
    const NonStandardFaucetFactory = (await ethers.getContractFactory(
      "NonStandardFaucetToken"
    )) as NonStandardFaucetToken__factory;
    const nonStandardToken = await NonStandardFaucetFactory.deploy(
      1000e6,
      "Tether",
      6,
      "USDT"
    ...
    await nonStandardToken
      .connect(alice)
      .transfer(sweepableBridgeReceiver.address, transferAmount);

    const oldBridgeReceiverBalance = await nonStandardToken.balanceOf(
      sweepableBridgeReceiver.address
    );
    const oldTimelockBalance = await nonStandardToken.balanceOf(
    ...
    await proposeAndExecute(sweepableBridgeReceiver, govTimelock, {
      ...
      calldatas: [
        utils.defaultAbiCoder.encode(
          ["address", "address"],
          [localTimelock.address, nonStandardToken.address]
        ),
      ],
    });

    const newBridgeReceiverBalance = await nonStandardToken.balanceOf(
      sweepableBridgeReceiver.address
    );
    const newTimelockBalance = await nonStandardToken.balanceOf(
      localTimelock.address
    );
  ...
  it("sweeps native token", async () => {
    ...
    await alice.sendTransaction({
      to: sweepableBridgeReceiver.address,
      value: transferAmount,
    });

    const oldBridgeReceiverBalance = await ethers.provider.getBalance(
      sweepableBridgeReceiver.address
    );
    const oldTimelockBalance = await ethers.provider.getBalance(
      localTimelock.address
    );
    ...
    const newBridgeReceiverBalance = await ethers.provider.getBalance(
      sweepableBridgeReceiver.address
    );
    const newTimelockBalance = await ethers.provider.getBalance(
      localTimelock.address
    );
  ...
  it("reverts if sweepToken is called by address other than local timelock", async () => {
    const { sweepableBridgeReceiver, signers } =
      await makeSweepableBridgeReceiver();
    const [alice] = signers;
    ...
    await expect(
      sweepableBridgeReceiver
        .connect(alice)
        .sweepToken(alice.address, USDC.address)
  ...
  it("reverts if sweepNativeToken is called by non-admin", async () => {
    ...
    // Alice sweeps ETH
    await expect(
      sweepableBridgeReceiver.connect(alice).sweepNativeToken(alice.address)
    ).to.be.revertedWith("custom error 'Unauthorized()'");
  });
});
